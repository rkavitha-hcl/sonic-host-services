    ...
    def execute_reboot(self, reboot_method):
        ...
        if reboot_method in REBOOT_METHOD_HALT_BOOT_VALUES:
           try:
               ...
               pmon_stopped = not is_container_running("pmon")
               syncd_stopped = not is_container_running("syncd")
               if not (pmon_stopped and syncd_stopped):
                   logger.error("%s: HALT reboot failed: pmon or syncd is still running", MOD_NAME)
                   self.populate_reboot_status_flag()
                   return
           except Exception as e:
               logger.error("%s: Error received when checking containter status for pmon or syncd [%s]:", MOD_NAME, str(e))
